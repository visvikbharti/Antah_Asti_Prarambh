...
**Date:** 2026-03-22
**Version:** Phase 2 (FoldX thermodynamic stability pending — expected early April 2026)
...
### FoldX Stability (`results/phase2/foldx/`) — PENDING

| File | Expected | Description |
|------|----------|-------------|
| `per_protein/{accession}.json` | 25,007 | Individual FoldX results per protein. Contains `total_energy` (ΔG in kcal/mol). |
| `foldx_stability_all.tsv` | 25,007 | Aggregated TSV with all proteins and their stability values. Will be generated by the collection job after all array tasks complete. |
...
1. **pLDDT ≠ stability.** AlphaFold pLDDT is model confidence, not thermodynamic stability. FoldX ΔG (pending) will address this.
